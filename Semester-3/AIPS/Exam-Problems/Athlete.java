...
public class Main {
    ...
    public static void competition(SLL<Athlete> prevWin, SLL<Athlete> currWin){
        SLLNode<Athlete> head1 = prevWin.getFirst();
        SLLNode<Athlete> head2 = currWin.getFirst();
        double max = 0;
        while(head1!=null){
            if(head1.element.getTime()>max){
                max = head1.element.getTime();
            }
            head1 = head1.succ;
        }
        while(head2!=null){
            if(head2.element.getTime()>max){
                currWin.delete(head2);
            }
            head2 = head2.succ;
        }
    }
