...
public class Main {
    ...
    public static void forum(SLL<Discussion> health, SLL<Discussion> finance){
        SLLNode<Discussion> headFinance = finance.getFirst();
        SLLNode<Discussion> tempFinance = null;
        int min = 9999;
        while(headFinance!=null){
            int minImportance = headFinance.element.getPopularity()*headFinance.element.getPopularity();
            if(minImportance<min){
                min = minImportance;
                tempFinance = headFinance;
            }
            headFinance = headFinance.succ;
        }
        SLLNode<Discussion> headHealth = health.getFirst();
        SLLNode<Discussion> tempHealth = null;
        int max = 0;
        while(headHealth!=null){
            int maxImportance = headHealth.element.getPopularity()*headHealth.element.getPopularity();
            if(maxImportance>max){
                max = maxImportance;
                tempHealth = headHealth;
            }
            headHealth = headHealth.succ;
        }
        if (tempFinance != null && tempHealth != null) {
            finance.delete(tempFinance);
            finance.insertLast(tempHealth.element);
        }
    }
