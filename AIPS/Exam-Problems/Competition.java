...
public class Main
{
    public static void competition(SLL<Competitor> prev, SLL<Competitor> current) {
        SLLNode<Competitor> head1 = prev.getFirst();
        SLLNode<Competitor> head2 = current.getFirst();
        double sum = 0;
        double avg = 1;
        while(head1!=null){
            sum += head1.element.getPoints();
            head1=head1.succ;
        }
        ...
        while(head2!=null){
            if(head2.element.getPoints()<avg){
                current.delete(head2);
            }
            head2 = head2.succ;
        }
    }
